  ...
  if (empty($isi)) {
    ...
    if ($gambar != "") {
      ...
      if (in_array($ekstensi, $ekstensi_diperbolehkan) === true  && move_uploaded_file($file_tmp, '../assets/images/artikel/' . $gambarbaru)) {
        ...
        $ubah = mysqli_query($koneksi, "UPDATE artikel SET judul='$judul', asal='$asal', kategori='$kategori', penulis='$penulis', tanggal='$tanggal', preview='$preview', isi='$isi', link='$link', gambar='$gambarbaru' WHERE id_artikel='$id'");
        if ($ubah) {
          header('location: wisata.php?edit=1');
        }
      }
    } else if ($gambar = "") {
      mysqli_query($koneksi, "UPDATE artikel SET judul='$judul', asal='$asal', kategori='$kategori', penulis='$penulis', tanggal='$tanggal', isi='$isi', preview='$preview', link='$link' WHERE id_artikel='$id'");
      header('location: wisata.php?edit=1');
    } else {
      mysqli_query($koneksi, "UPDATE artikel SET judul='$judul', asal='$asal', kategori='$kategori', penulis='$penulis', tanggal='$tanggal', isi='$isi', preview='$preview', link='$link' WHERE id_artikel='$id'");
      header('location: wisata.php?edit=1');
